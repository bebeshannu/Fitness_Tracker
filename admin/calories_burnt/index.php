...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title">Calories Burnt</h3>
        ...
        <div class="container-fluid">
			<table class="table table-bordered table-stripped">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT r.*,c.category,c.balance from `running_balance` r inner join `categories_2` c on r.category_id = c.id where c.status= 1 and r.balance_type = 2 order by unix_timestamp(r.date_created) desc");
...
<script>
	$(document).ready(function(){
		$('#manage_expense').click(function(){
			uni_modal("<i class='fa fa-plus'></i> Add New Activity",'expense/manage_expense.php')
		})
		$('.manage_expense').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Update Activity",'expense/manage_expense.php?id='+$(this).attr('data-id'))
		})
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this Activity permanently?","delete_expense",[$(this).attr('data-id'),$(this).attr('data-category_id')])
		})
