...
Class Master extends DBConnection {
	...
	function delete_category_2(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `categories_2` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
	...
	function save_category_2(){
		...
		$check = $this->conn->query("SELECT * FROM `categories_2` where `category` = '{$category}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `categories_2` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `categories_2` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
	...
	function save_budget(){
		...
		if($save){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success'," Calories consumed successfully saved.");
			}else{
				$resp['status'] = 'failed';
	...
	function save_expense(){
		...
		if($save){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success'," Calories burnt successfully saved.");
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $update_balance;
			}
